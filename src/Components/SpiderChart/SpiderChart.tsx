...
const concentricCircles = (svg, width, ticks = [2, 4, 6, 8, 10]) => {
  ...
  ticks.forEach((t) =>
    svg
      ...
      .style(
        "stroke-dasharray",
        t === ticks[ticks.length - 1] ? "0, 0" : "2, 2"
      )
...
const getPathCoordinates = (data, data_point, offset = 0, width) => {
  ...
  for (var i = 0; i < features.length; i++) {
    ...
    let angle = Math.PI / 2 + (2 * Math.PI * i) / features.length;
    coordinates.push({
      ...angleToCoordinate(angle, data_point[ft_name] + offset, width),
...
const SpiderChart = ({
  ...
  useEffect(() => {
    let widthSvg = width
      ? width
      : document.querySelector("#spider-chart").getBoundingClientRect().width;
    let heightSvg = height ? height : widthSvg;
    console.log(widthSvg, heightSvg);
    let svg = d3
      .selectAll("#spider-chart")
      .append("svg")
      .attr("width", widthSvg)
